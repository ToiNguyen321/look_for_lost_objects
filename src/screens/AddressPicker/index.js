...
export default AddressPicker = ({navigation, route, ...props}) => {
  ...
  useEffect(() => {
    const array1 = Array.from(
      filter(
        address_data_full_alias,
        (x) => x.includes(change_alias(address2)),
        25,
      ),
    );
    const array2 = Array.from(
      filter(address_data_alias, (x) => x.includes(change_alias(address2)), 25),
    );
      ...
      <View style={styles.container}>
        <SInput
          ...
          onChangeText={(text) => {
            setErrors({
              ...errors,
              address: text ? null : 'Vui lòng nhập đúng địa chỉ',
            });
            setAddress(text);
          }}
        ...
        <SInput
          ...
          onChangeText={(text) => {
            setErrors({
              ...errors,
              address2: validate.length(text)
                ? null
                : 'Vui lòng nhập đúng Quận/Huyện và Tỉnh/Thành Phố',
            });
            setAddress2(text);
          }}
          ...
          ListFooterComponent={() => (
            <View>
              <Text style={styles.footerText}>
                ......{'\n\n'}Để tìm chính xác địa chỉ, vui lòng nhập địa chỉ
                của bạn vào ô tìm kiếm xã, quận, tỉnh,...
              </Text>
            </View>
          )}
        />
